...
RSpec.describe "get a single user trainings", :type => :request do
  ...
  let!(:player) {Player.create(id: 1, name: 'Mark J Williams', 
    nationality: 'Wales', 
    born: '1975-03-21', 
    firstseason: '1992', 
  ...
  before {
    sign_in user
    user.trainings.create!(date: '2022-05-03', location: 'The Crucible Theatre, Sheffield, EN', player_id: 1)
  ...
  it 'returns all user trainings' do
    expect(JSON.parse(response.body).size).to eq(1)
  end 

  it 'returns the date' do
    expect(JSON.parse(response.body).first['date']).to eq('2022-05-03')
  end

  it 'returns the location' do
    expect(JSON.parse(response.body).first['location']).to eq('The Crucible Theatre, Sheffield, EN')
  end

  it 'returns the player ID' do
    expect(JSON.parse(response.body).first['player_id']).to eq(1)
  end

  it 'returns the user ID' do
    expect(JSON.parse(response.body).first['user_id']).to eq(1)
  end
end